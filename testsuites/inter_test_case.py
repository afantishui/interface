...
# logger = Logger(logger="inter_test_case").getlog()

filepath = 'D:\GitHub\interface\\testcases\\inter_testcase.xlsx'
print(filepath)
...
def test_interface():
	...
	for i in range(len(case_url_list)):
		...
		apicode = api.getcode()
		apijson = api.getJson()
		print('inputdata>参数:%s, url:%s ,返回:%s,预期:%s'%(case_con_list[i],case_url_list[i],apijson,case_expect_list[i]))
